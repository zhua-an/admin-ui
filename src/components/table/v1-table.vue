<template>
  <div class="v1-table">
    <template :table="table">
      <el-table
        v-loading="vTable.loading"
        :show-summary="vTable.hasShowSummary"
        :summary-method="vTable.getSummaries"
        :ref="vTable.ref"
        :data="vTable.data"
        tooltip-effect="dark"
        :border="vTable.border"
        style="width: 100%"
        ...
        @row-click="rowClick">
        <el-table-column
          v-if="vTable.hasIndex"
          type="index"
          width="50">
        </el-table-column>
        <el-table-column
          v-if="vTable.hasSelect"
          type="selection"
          width="55">
        </el-table-column>
        <el-table-column type="expand" v-if="vTable.hasExpand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              <el-form-item :label="item.label" v-for="(item,index) in vTable.expands" :key="index">
                <span>{{ props.row[item.prop] }}</span>
              </el-form-item>
            </el-form>
          </template>
        </el-table-column>
        <template v-for="(item,index) in vTable.column">
          <el-table-column
        ...
        <el-table-column
          ...
          :label="vTable.operation.label"
          :align="vTable.operation.align"
          :width="vTable.operation.width ? vTable.operation.width : ''"
          :min-width="vTable.operation.minWidth ? vTable.operation.minWidth : ''"
          :class-name="vTable.operation.className"
          v-if="vTable.hasOperation">
          <template slot-scope="scope">
            <el-button
              v-for="(item,index) in vTable.operation.data"
...
//          </template>
    ...
    computed: {
      vTable: function() {
        return Object.assign({}, this.tableProps, this.table);
      }
    },
    methods: {
      ...
      handleOperation(a, b, id) {
        const data = this.vTable.operation.data;
